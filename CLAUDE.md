...
| `ignoreDotFolders`                      | boolean | `true`                                                                                                                  | Ignore all folders whose names start with `.` (e.g. `.github`, `.vscode`, `.husky`). Set to `false` to include them. |
...
`extension.ts` combines `defaultIgnores`, `additionalIgnores`, and (when `ignoreDotFolders` is `true`) the pattern `.*/` into `allIgnores`, then passes that as the `additionalIgnores` argument to all file-helper functions. This keeps `ignoreHelper.ts` free of VS Code dependencies.
